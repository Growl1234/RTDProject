...
[Vaspkit程序](https://vaspkit.com/)可以在生成输入文件时根据你期望的精度设置自动生成一个k点（多数情况偏大；如果要使用vaspkit生成k点，对于一般问题个人建议步骤为“102-->1-->0.04”）。理论上讲，晶胞越大，需要考虑的k点数目越小；k点数目无穷大时的计算结果和扩胞扩到无限大时的计算结果是等价的。经验上讲，假设a为某方向的晶胞边长，k为该方向考虑的k点数目，则分子晶体的k_*a一般不低于15Å，含d族过渡金属的体系k_*a一般不低于30Å。
