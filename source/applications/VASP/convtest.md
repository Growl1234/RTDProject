...
VASP中的平面波截断能（ENCUT）和k点数目是实际计算的两个非常重要的影响计算精度的设置。这两个参数设得太小显然会让结果太不精确，但设得太大对精度进一步改进不明显却耗时暴增，因此不能盲目设大。

[Vaspkit程序](https://vaspkit.com/)可以在生成输入文件时根据你期望的精度设置自动生成一个k点（多数情况偏大；如果要使用vaspkit生成k点，对于一般问题个人建议步骤为“102-->1-->0.04”）。理论上讲，晶胞越大，需要考虑的k点数目越小；k点数目无穷大时的计算结果和扩胞扩到无限大时的计算结果是等价的。经验上讲，假设a为某方向的晶胞边长，k为该方向考虑的k点数目，则分子晶体的k \cdot a一般不低于15Å，含d族过渡金属的体系k \cdot a一般不低于30Å。
...
显然，是否考虑k点、SCF是否收敛等影响能量及其精度的因素多多少少会对结果造成影响。由于VASP的算法原因，在做ENCUT收敛性测试时不建议为节省时间而放宽SCF收敛限，更不能为此有意减小SCF步数上限甚至减到1（而CP2K可以）；至于是否可以为节约时间而选较粗的k点或者仅考虑gamma点，我还没有测试过，等测试了会更新这里的说明。
...
做k点随能量的收敛性测试的bash脚本的基本思路跟上面一样，大体模仿着写即可。

需要注意的是，由于k点设定是包含空格的，因此不应该像上面的脚本那样设置要测试的k点；建议单另添一个用于写要测试的k点的纯文本文档（不同k点分行写），然后在bash脚本里面正确引用之。另外，k点的收敛性测试应当让每一个分别任务的SCF都按照一般单点能计算问题的标准正常收敛。ENCUT的大小可能对结果造成影响，如果先做k点收敛性测试的话，先自己按经验做法选一个合适的ENCUT值。
