...
* `-j N`即并行设置，“N”就是并行核数，如果不设置，脚本将通过`nproc`检测你的电脑CPU的可用线程数并将其设为默认值（然而这一命令受到额外的cgroup限制）。可惜的是，目前Spack无法实际控制编译依赖所用的`make -j N`，这会导致多个程序安装步骤同时执行且同时请求所设置的核心数，导致严重的资源争抢和疯狂的内存挤占，后者可能导致构建失败以及系统临时失去响应。
