...
如果正确加载，会输出显卡信息和使用显卡的进程信息。你也可以通过“pip install nvitop”安装nvitop工具来实时监测显卡占用情况。
...
2. “No devices were found”，即该指令与驱动程序正确对接了，但却完全没有识别到显卡设备。对于这个问题，我姑且列出最可能的几种情况：一根筋不信邪地通过“.run”驱动安装程序装了驱动导致的，解决办法是卸载掉从头再来；如果你的显卡不是太老，应当尽量选择开源模块（第一部分有提到）；对于版本10.0，应当先检查以下自己有没有更新到最新的内核版本，更新并重启以进入新内核后建议再执行一下“nvidia-modprobe && nvidia-modprobe -u”然后重启电脑，问题大概率会得到解决。
