...
（2）依次按下图操作：（记得同时看下面的补充文字；保留默认选项即可的页面没有给出）
...
只要按照我所提到的选项安装好，Rocky Linux会自带open-vmtools，可以实现与Windows的一些基本互通功能，比如剪贴板可以共享。然而，想要更方便地在Windows主机和虚拟机之间传输一些大文件和数据，设置共享文件夹是非常有必要的。以下简要说一说共享文件夹的设置方法（不要胡乱找一些别的网上教程，很多要么是错误的要么是有冗余步骤的）：

**(1) 在VMware应用内开启共享文件夹**

（待完善）

**(2) 在Rocky Linux中载入共享文件夹**（参考[https://knowledge.broadcom.com/external/article/313233/how-to-configure-vmware-tools-shared-fol.html](https://knowledge.broadcom.com/external/article/313233/how-to-configure-vmware-tools-shared-fol.html)）
