...
***2026-01-09补充：P4对应操作中，如果你的CPU是大小核配置（比如12代开始的酷睿系列CPU），那么只分配大核一般也够用，把小核也分配进去通常并不会比不分配小核快多少（实测Gaussian计算任务得出的结论）；此时内存可以按照两倍分配核数来给。这样做的前提是你已经在Windows中调整了调度策略，关于这一点我将来会加一篇新文章略作阐述；大家可以先看[https://www.bilibili.com/video/BV1e94y1L7Ds](https://www.bilibili.com/video/BV1e94y1L7Ds)了解如何调整系统设置中的调度策略，对于大小核CPU应将里面对应选项设置成“优先使用高性能核心”。***
